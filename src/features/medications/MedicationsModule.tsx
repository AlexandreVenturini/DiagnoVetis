...
export function MedicationsModule() {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const medicamento = new Medicamento(id, form.commercialName.trim(), form.activeIngredient.trim(), form.notes.trim(), concentrationValue, concentrationUnit, form.frequency.trim(), form.route.trim(), form.dosage.trim())
    setSaving(true)
    try {
      await medicamentoService.adicionarMedicamento(medicamento)
      const medication: Medication = { ...form, id, indications: splitList(form.indications), contraindications: splitList(form.contraindications), concentration: `${concentrationValue} ${concentrationUnit}`, concentrationMgMl: concentrationUnit.toLocaleLowerCase('pt-BR').replaceAll(' ', '') === 'mg/ml' ? concentrationValue : null }
      setItems((current) => [...current, medication]); setSelectedId(id); setScreen('browse'); setForm(EMPTY_FORM)
    } catch (cause) {
  ...
  return <section className="medications-module">
    ...
    </div><div className="form-actions"><button className="primary-button" type="submit" disabled={saving}>{saving ? 'Cadastrando...' : 'Cadastrar medicamento'}</button><button className="secondary-button" type="button" onClick={() => setScreen('browse')} disabled={saving}>Cancelar</button></div></form> : <div className="medication-browser"><div className="medication-list">{filtered.map((item) => <button key={item.id} className={`medication-card${selectedId === item.id ? ' selected' : ''}`} onClick={() => setSelectedId(item.id)}><span className="medication-card-icon"><PillIcon /></span><span><strong>{item.commercialName}</strong><small>{item.activeIngredient}</small>{item.indications.length > 0 && <span className="medication-indications"><b>Indicações:</b> {item.indications.join(', ')}</span>}<span className="medication-tags"><em>{item.dosage}</em><em>{item.frequency}</em></span></span></button>)}{!loading && filtered.length === 0 && <div className="medication-empty-list">Nenhum medicamento encontrado.</div>}{loading && <div className="medication-empty-list">Carregando medicamentos...</div>}</div><MedicationDetails medication={selected} /></div>}
  </section>
}

...
export function DashboardHome({ dogs, onOpenModule, onNewDog, onNewAppointment }: DashboardHomeProps) {
  ...
  return <section className="dashboard-home">
    ...
    <div className="dashboard-stats"><StatCard tone="green" label="Consultas hoje" value={todayAppointments.length} detail={pending.length ? `${pending.length} aguardando ação` : 'Nenhuma pendência na fila'} /><StatCard tone="teal" label="Pacientes cadastrados" value={dogs.length} detail="Animais disponíveis no cadastro" /><StatCard tone="amber" label="Lembretes pendentes" value={pendingReminders.length} detail={pendingReminders.length ? 'Retornos ou vacinações vencendo' : 'Nenhum lembrete em atraso'} /><StatCard tone="red" label="Alertas clínicos" value={clinicalAlerts} detail={clinicalAlerts ? 'Itens exigem acompanhamento' : 'Acompanhamento em dia'} /></div>

    <div className="dashboard-main-grid"><section className="dashboard-panel dashboard-agenda"><div className="dashboard-panel-heading"><div><span>Operação do dia</span><h2>Agenda e fila de atendimento</h2></div><button onClick={() => onOpenModule('appointments')}>Ver agenda completa <b>→</b></button></div>

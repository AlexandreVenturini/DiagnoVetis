...
export function RecordsModule() {
  ...
  if (!latestRecord) return <section className="record-details-module">
    ...
    <div className="empty-appointments">Nenhum registro clínico encontrado para este paciente.</div>
  </section>
  ...
  return <section className="record-details-module">
    ...
    <form className="animal-record-form" ref={recordFormRef}>
      ...
      <div className="animal-record-grid">
        ...
        <RecordSection number="8" title="Tratamento" className="third" onAddRow={() => setTreatmentRows((count) => count + 1)} onRemoveRow={treatmentRows > 1 ? () => setTreatmentRows((count) => Math.max(1, count - 1)) : undefined}><table><thead><tr><th>Medicamento</th><th>Dose</th><th>Frequência</th></tr></thead><tbody>{latestRecord.prescriptions.map((item, index) => <tr key={`${item}-${index}`}><EditableCells values={[item, '', '']} /></tr>)}{Array.from({ length: treatmentRows }, (_, index) => <tr key={`tratamento-extra-${index}`}><EditableCells count={3} /></tr>)}</tbody></table><div className="paper-fields single compact"><label>Procedimentos<input defaultValue={latestRecord.conduct} /></label><label>Orientações ao tutor<input /></label></div></RecordSection>
